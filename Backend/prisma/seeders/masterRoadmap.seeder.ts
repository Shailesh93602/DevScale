  ...
  try {
    ...
    for (const [rmIdx, rmDir] of roadmapDirs.entries()) {
      ...
      for (const [mcIdx, mcDir] of mcDirs.entries()) {
        ...
        if (!mcMeta) { console.warn(`  ⚠️  Skipping MainConcept ${mcDir} — missing meta.json`); continue; }
        ...
        for (const [sIdx, sDir] of subjectDirs.entries()) {
          ...
          if (!sMeta) { console.warn(`    ⚠️  Skipping Subject ${sDir} — missing meta.json`); continue; }
          ...
          for (const [tIdx, tDir] of topicDirs.entries()) {
            ...
            if (!tMeta) { console.warn(`      ⚠️  Skipping Topic ${tDir} — missing meta.json`); continue; }
