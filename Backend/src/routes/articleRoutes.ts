...
export class ArticleRoutes extends BaseRouter {
  ...
  protected initializeRoutes(): void {
    ...
    this.router.post('/status', authMiddleware, authorizeRoles('ADMIN', 'MODERATOR'), validateRequest(updateArticleStatusSchema), this.articleController.updateArticleStatus);
    this.router.post('/:id/moderation', authMiddleware, authorizeRoles('ADMIN', 'MODERATOR'), validateRequest(articleIdParamSchema, 'params'), validateRequest(updateModerationNotesSchema), this.articleController.updateModerationNotes);
    this.router.post('/:id/update', authMiddleware, authorizeRoles('ADMIN', 'MODERATOR'), validateRequest(articleIdParamSchema, 'params'), validateRequest(updateArticleContentSchema), this.articleController.updateArticleContent);
  }
}
