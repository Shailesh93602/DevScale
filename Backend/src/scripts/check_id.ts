...
async function main() {
    ...
    const roadmapMainConcept = await prisma.roadmapMainConcept.findUnique({ where: { id } });
    console.log('RoadmapMainConcept:', roadmapMainConcept);

    const mainConceptSubject = await prisma.mainConceptSubject.findUnique({ where: { id } });
