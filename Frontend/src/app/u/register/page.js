...
export default function Register() {
    ...
    const handleSubmit = async (e) => {
        ...
        if (!terms.checked) return terms.focus();
        ...
        if (json.success) console.log(json.message);
    }
    return (
                    ...
                    <div className="p-6 space-y-4 md:space-y-6 sm:p-8">
                        ...
                        <form className="space-y-4 md:space-y-6" onSubmit={handleSubmit}>
                            <div>
                                ...
                                <input type="text" name="username" id="username" className="bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5" placeholder="john1998" required="" value={username} onChange={(e) => setUsername(e.target.value)} />
                            </div>
                            <div>
                            ...
                            <div>
                                <label htmlFor="repeatPassword" className="block mb-2 text-sm font-medium text-gray-900">Confirm password</label>
                                <input type="password" name="repeatPassword" id="repeatPassword" placeholder="••••••••" className="bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5" required="" value={repeatPassword} onChange={(e) => setRepeatPassword(e.target.value)} />
                            </div>
                            ...
                            <p className="text-sm font-light text-gray-500">
                                Already have an account? <a href="/u/login" className="font-medium text-primary-600 hover:underline">Login here</a>
                            </p>
                        </form>
                    </div>
                </div>
