...
const Navbar = () => {
  ...
  const showPublicNavbar = !isAuthenticated;
  ...
  return (
    ...
    >
            ...
            {showPublicNavbar ? (
              ...
              <div className="relative">
                <button
                  ...
                  className={cn(
                    'group relative flex h-[42px] items-center gap-2.5 rounded-full border border-border/60 bg-white/5 px-1.5 shadow-sm transition-all duration-300 hover:bg-white/10 focus:outline-none dark:bg-zinc-900/50 dark:hover:bg-zinc-800/80',
                    dropdownOpen &&
      ...
      <div
        className={cn(
          'grid border-t border-border/40 bg-background/95 backdrop-blur-xl transition-all duration-300 ease-in-out lg:hidden',
          isDesktop && 'hidden',
            ...
            {navList.map((navItem, index) =>
              navItem.path === '/battle-zone' ? (
                <div key={index} className="relative">
                  <button
                    onClick={toggleBattleZone}
                    className="flex w-full items-center justify-between rounded-xl px-4 py-3 text-left text-[14px] font-medium text-foreground/80 transition-colors hover:bg-foreground/5 hover:text-foreground focus:outline-none"
                  >
                    {navItem.label}
                    <FiChevronDown
                      className={cn(
