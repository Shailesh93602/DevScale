...
const AchievementCard = ({ title, description, value }) => {
    const isCompleted = value === 100;
    const imageClass = value !== 100 ? 'filter grayscale' : '';

    // Determine level based on progress value
    let level;
    if (value >= 90) {
        level = 'complete';
    } else if (value >= 70) {
    ...
    return (
        <div className="bg-white shadow-lg rounded-lg p-6 transition duration-300 ease-in-out transform hover:scale-105 hover:bg-gray-100 cursor-pointer">
            <div className="flex items-center space-x-4">
                ...
                <div className={`w-12 h-12 flex items-center justify-center bg-blue-500 text-white rounded-full ${imageClass}`}>
                    <svg
