...
import { useCodeReview, type CodeReviewResult } from './useCodeReview';
...
describe('useCodeReview', () => {
  it('posts to the review endpoint with the submission id and returns the review', async () => {
    ...
    expect(execute).toHaveBeenCalledWith({}, undefined, { submissionId: 'sub-1' });
  ...
  it('throws with the API message when the response is unsuccessful', async () => {
    ...
    expect((err as Error).message).toBe('AI features are not configured');
  });
});
