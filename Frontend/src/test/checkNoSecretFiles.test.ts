...
// The guard lives at the repository root; this suite runs from Frontend/.
...
const GUARD = path.resolve(REPO_ROOT, 'scripts/check-no-secret-files.mjs');
...
describe('check-no-secret-files', () => {
  ...
  it('is satisfied by this repository itself', () => {
    const { code, out } = run({ dir: REPO_ROOT, env: process.env });
    expect(out).not.toMatch(/FAILED/);
    expect(code).toBe(0);
  });
});
