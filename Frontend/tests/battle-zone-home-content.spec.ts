...
test.describe('Battle Zone Home Experience', () => {
  test('home page shows actionable battle discovery content', async ({ page }) => {
    ...
    await expect(page.getByRole('heading', { name: 'Battle Zone Arena' })).toBeVisible();
    await expect(page.getByPlaceholder('Search battles...')).toBeVisible();
    await expect(
      page.getByRole('button', { name: /Create Battle|Create Your First Battle/i }).first(),
    ).toBeVisible();
  });
});
